...
def send(data: dict):
    print(requests.post(SPLUNK_URL, headers=HEADERS, json=dict(event=data), verify=False).text)
    ...
    with open(csvFilePath, encoding='utf-8') as csvf:
        ...
        for rows in csvReader:
            if len(rows['\ufeffתאריך רכישה'].replace(' ','')) > 1:
                ...
                key = rows['\ufeffתאריך רכישה']
                eng = { "Price": rows['סכום חיוב'],
                        "Business": rows['שם בית עסק'].replace('"','').replace("'","").replace(' ','_'),
